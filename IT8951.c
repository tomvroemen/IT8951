...
void LCDWriteCmdCode(uint16_t usCmdCode)
{
	...
	printf("CMD %#04x %#04x %#04x %#04x\r\n",wPreamble>>8, wPreamble, usCmdCode>>8, usCmdCode);
}
...
void IT8951_RIPPLE_Info_L(uint32_t x, uint32_t y,char *string, uint8_t scale)
{
	IT8951LdImgInfo stLdImgInfo;
	IT8951AreaImgInfo stAreaImgInfo;
	EPD_Text(0,  0, (uint8_t*)string,0x00, 0xff, scale);
	...
	stLdImgInfo.ulImgBufBaseAddr = gulImgBufAddr;
	//Set Load Area
	stAreaImgInfo.usX      = x-8*scale*strlen(string);
	stAreaImgInfo.usY      = y;
	stAreaImgInfo.usWidth  = gstI80DevInfo.usPanelW;
	stAreaImgInfo.usHeight = 16*scale;
...
void IT8951_BMP_Example(uint32_t x, uint32_t y,char *path)
{
	...
	EPD_DrawLine(375, 500, 500, 566, 0x00);
